...
if __name__ == "__main__":
	...
	while True:
		...
		for x, y, win in sliding_window(gray, int(WINDOW_SIZE[0]/2), WINDOW_SIZE):
			#red = win[:, :, 2];
		...
		# параметрическое определение нормального распределения
			...
			for i in range(int(np.ceil(mu + 4*sigma)), 256):
				rtable[i] = i
			#table = np.dstack((identity, identity, rtable))
			cv2.LUT(win, rtable, win)
	...
	# hsv = cv2.LUT(hsv, table)
		...
		cv2.imshow("blur", cv2.resize(bin, (1280, 720)))
		if cv2.waitKey(1) == 27:
			break
