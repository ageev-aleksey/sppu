...
QJsonObject QSddModelJsonSerializer::stateSerialize(const SddModel::State &state) {
    QJsonObject json;
...
SddModel::State QSddModelJsonSerializer::stateDeserialize(const QString &str) {
    QByteArray bytes;
    bytes.append(str);
...
QJsonObject QSddModelJsonSerializer::inputSerialize(const SddModel::Input &input) {
    QJsonObject json;
...
SddModel::Input QSddModelJsonSerializer::inputDeserialize(const QString &str) {
    QByteArray bytes;
    bytes.append(str);
    QJsonDocument doc = QJsonDocument::fromJson(bytes);
    if(!doc.isObject()) {
...
QByteArray QSddModelJsonSerializer::write(SddModelDescriptor &&object) {
...
SddModelDescriptor QSddModelJsonSerializer::read(const QByteArray &data) {
    return SddModelDescriptor();
}

QJsonObject QSddModelJsonSerializer::parametersDeserialize(const SddModel::Parameters &parameters) {
    QJsonObject res;
...
QSddModelJsonSerializer::QSddModelJsonSerializer() : Format<SddModelDescriptor>("json") {}
